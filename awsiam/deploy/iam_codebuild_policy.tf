data "aws_iam_policy_document" "codebuild_policy_document" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:cognito-identity:${local.region}:${local.account_id}:identitypool/*"
    ]
  }

  statement {
    sid = "ManageIdentityAuthRoles"
    effect = "Allow"
    actions = [
      "iam:CreateRole",
      "iam:GetRole",
    ...
    resources = [
      "arn:aws:iam::${local.account_id}:role/${local.name_prefix}-authenticated"
    ]
  }

  statement {
    # this allows us to create test users
    sid = "ManageUserPoolTestUsers"
    effect = "Allow"
    actions = [
